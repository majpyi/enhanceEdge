...
gray = np.loadtxt("gradient_"+test.src+".csv", delimiter=",")
...
NMS_re = np.zeros((W2, H2))
for i in range(1, W2 - 1):
    for j in range(1, H2 - 1):
        if (NMS[i, j] >5 ):
            NMS_re[i, j] = 255

cv2.imwrite("NMS.jpg", NMS_re)
np.savetxt('NMS.csv', NMS_re, fmt="%d", delimiter=',')
...
cv2.imwrite("CannySelf.jpg", DT)
np.savetxt('CannySelf.csv', DT, fmt="%d", delimiter=',')
