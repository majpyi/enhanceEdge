...
#             b_index.append(ii)
#
#     #     if line.count(max(line)) == 1:#差值最大的边只有一条
#     #         del line[large1_index]
#     #     else:#差值最大的不止一条
#     #         for i in range(line.count(max(line)))
#     #             line_temp = line[:]
#     #             del_large_index.append()
#     #     print(line)
#     #     large2_index = line_1.index(max(line))  # 避免重复值位置重复！
#     #     large2_index = len(point_list) - 1 - large2_index
#     #     index2 = line.index(max(line))
...
#     #             b.append(i)
#     #     for i in A:
#     #         point_list.remove(i), A, point_list,
#     return energy, a, b, a_index, b_index
def least_energy(point_list):
    line = []  # 存放边的能量
    for p in range(-1, len(point_list) - 1):
    ...
    if flag == 0:  # 所有一个值
        ...
        if max_count==1:
            ...
            max_diff_2 = line_1[max_count-1]
            max_count_2 = line_1.count(max_diff_2)
        #print('max_diff_2 count_2',max_count,max_diff_2,max_count_2)
    for i in range(len(line)):
        if line[i] == max_diff_2:  # 如果是最大的值
            del_large_index_2.append(i)
            start = i + 1
    ...
    # print("!!!del_large_index,  del_large_index_2 ", del_large_index, del_large_index_2)
    energy = sum(line) - max_diff - max_diff_2
    #print('energy',energy,sum(line),max_diff,max_diff_2)
    a_b_diff = []  # 存储每种搭配下ab两区的差值
    if (len(del_large_index) >= 2):  # 最大的值不止一个
        for i in del_large_index:
            ...
            for j in del_large_index[mm + 1:]:
                ...
                # print(temp_a, temp_b)
                if len(temp_a)==0 :
                    diff = abs(  sum(temp_b) / len(temp_b))
                elif len(temp_b)==0:
                    diff=abs(sum(temp_a) / len(temp_a))
                else:
                    diff = abs(sum(temp_a) / len(temp_a) - sum(temp_b) / len(temp_b))
                a_b_diff.append((diff, i, j))
    else:
...
def noise_array(gray, ccount,th, num=8):
    #     count=0
    length = gray.shape[0]
    width = gray.shape[1]
    ...
    start1 = time.clock()
    score_array_1 = score_new(gray, ccount,th, num)[0]  # 有值
...
# 遍历灰度矩阵并计数标记，噪声点+0，内部点+1，小边点+10，大边点+100,num为邻域个数,count为每次取噪声个数
def score_new(gray, count,th, num=8):
    # th = 3
    scores = np.zeros((gray.shape[0], gray.shape[1]))  # 初始化为0
    total_noise = []
    for k in range(1, gray.shape[0] - 1):
        for l in range(1, gray.shape[1] - 1):
            noise, a, b = point_classification_1(gray, k, l, count, num)  # 根据中心像素点得到八邻域中的噪声，a,b区坐标
            total_noise.extend(noise)
            num_a = []
            num_b = []
            for i in a:
            ...
            # if avg_a - avg_b > th:  # a区的点设为大边点, b区为小边点, 5为假设！！！！！！！！！！！！！！！！！
                for i in a:
                    scores[i[0]][i[1]] = scores[i[0]][i[1]] + 100
                    if i[0]==xx and i[1]==yy:
                        print("大")
                for i in b:
                    scores[i[0]][i[1]] = scores[i[0]][i[1]] + 10
                    if i[0]==xx and i[1]==yy:
                        print("小")
            elif (min(num_b) >= max(num_a)) and (abs(avg_b - avg_a) > th):  # b区的点设为大边点，a区为小边点
...
# 得到：三个噪声点，A,B区与中心像素的坐标
# 如果噪声相同值多个,会删除那个比较多的区域的像素点
def point_classification_1(gray, i, j, count, num=8):
    list_point = point_list(gray, i, j, num)[0]  # 初始的八邻域
    # print(list_point)
    # print(1)
    e_point_1 = list_point[:]  # 副本
