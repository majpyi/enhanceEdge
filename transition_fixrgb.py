...
np.savetxt(outpath+ src +"ththth"+str(th)+"noisenum"+str(noise_num)+".csv", guodu, fmt="%d", delimiter=',')
cv2.imwrite(outpath+ src +"ththth"+str(th)+"noisenum"+str(noise_num)+".jpg", guodu)

# 查看过渡区域周围八邻域的像素点,查看是否有非过渡区的点,把非过渡区的点中最接近过渡区的像素点的值付给过渡区的这个像素点
def change_tran(raw2, guodu,re):
    tag = 0
...
# guodu = np.loadtxt("D:\\dilat.csv", dtype=np.int, delimiter=",", encoding='utf-8')
#
#
