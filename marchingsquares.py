...
def traverse(matrix,src):
    ...
    for jj in range(len(xxx)):
        ...
        # plt.scatter(xxx[jj], yyy[jj], color='black', lw=0.1)
    ax = plt.gca()
    ax.invert_yaxis()  # y轴反向
    plt.xticks([])
    plt.yticks([])
    plt.axis('off')
    plt.savefig("D:\\"+str(src)+str(config.threshold_march), dpi=500)  # 指定分辨率保存
    plt.show()


#遍历所有像素点，画图
def traverse1(matrix , src):
    w,h = matrix.shape
    mid_points = []#标记点
    spe_indexs = []#为跟踪做准备
#     indexs = []
                    ...
                    if len(mid)>2:#特殊情况
                        ...
                        for ii in range(len(mid)):
                            ...
                            if mid[ii] not in mid_points:
                                ...
                                print("!!!!!!!!!!!!没加进去的",mid[ii])
                                mid_points.append(mid[ii])
                                mm = find_next_sq(matrix,spe[ii][0],spe[ii][1],spe[ii][2][0],spe[ii][2][1],spe[ii][3][0],spe[ii][3][1])
                                if mm is None:
                                    break
                                else:
                                    mix,line_index = mm
