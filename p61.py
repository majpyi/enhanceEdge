...
#
# 标记原始大小二维矩阵中的分割点的前后位正负值
# double_edge = np.zeros((raw2.shape[0], raw2.shape[1]))
# for m in range(0, len(re_weak)):
#     x1 = re_weak[m][0][0]
#     x2 = re_weak[m][1][0]
#     y1 = re_weak[m][0][1]
#     y2 = re_weak[m][1][1]
#     if raw2[x1,y1] > raw2[x2,y2]:
#         double_edge[x1,y1] = 1
#         double_edge[x2,y2] = -1
#     else:
#         double_edge[x1, y1] = -1
...
#         double_edge[x1, y1] = -2
#         double_edge[x2, y2] = 2
# np.savetxt("D:\\double_edge" + src + ".csv", double_edge, fmt="%d", delimiter=',')
...
# 只用来存储次明显分割点
gray2 = np.zeros((raw2.shape[0] * 2, raw2.shape[1] * 2))
for m in range(0, len(x)):
    xx = int(x[m] * 2)
    yy = int(y[m] * 2)
    fix_tag(gray2, xx, yy)

# gray_clone_find_guodu = p6rgb.merge_tag(gray2,gray1)
gray = np.zeros((raw2.shape[0], raw2.shape[1]))
